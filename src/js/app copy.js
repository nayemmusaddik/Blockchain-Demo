    ...
    App.contracts.Deal.deployed().then(function(instance) {
      ...
      for (i = 0; i < buyers.length; i++) {
        if (buyers[i] !== '0x0000000000000000000000000000000000000000') {
          $('.panel-car').eq(i).find('button').text('Chnage Owner');
          console.log(buyers[i]);
          //document.getElementById('owner_name').innerHtml = buyers[i];
          document.getElementsByClassName('car-owner')[i].innerHTML = buyers[i];
        }
      }
    }).catch(function(err) {
